...
export function buildYandexGptDescription(): INodeTypeDescription {
	return {
		...
		version: [1],
		subtitle:
			'={{$parameter.resource === "image" ? "Generate an image" : ($parameter.operation === "transcribe" ? "Transcribe a recording" : "Generate audio")}}',
		// Пустое: в n8n для community-нод этот текст дублируется в панели выбора действий
		// (ActionsMode → CommunityNodeInfo). Подробности — в README пакета / документации Яндекса.
		description: '',
		defaults: {
			name: 'YandexGPT',
		},
		...
		properties: [
			...
			...imageOperations,
			{
				...
				default: 'data',
				description:
					'Name of the binary property with audio. SpeechKit API v3. Formats: WAV, OggOpus, MP3. No 1 MB/30 s limit.',
				displayOptions: {
					show: {
						resource: ['audio'],
			...
			},
			{
				...
				description:
					'If set, used as modelUri as-is. Otherwise URI is built as art://{folderId}/{model}/{version} (see Yandex Cloud docs).',
				displayOptions: {
					show: {
						resource: ['image'],
						operation: ['generateImage'],
			...
			},
			{
				...
				default: '',
				description:
					'Maps to generationOptions.aspectRatio (widthRatio:heightRatio). Omit to let the API use defaults.',
