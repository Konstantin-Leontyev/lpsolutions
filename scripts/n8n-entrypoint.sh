#!/bin/sh
# Устанавливаем community node пакет в ~/.n8n/nodes (Via npm) и запускаем n8n.
...
if [ -f "$CUSTOM_PKG/package.json" ] && [ ! -f "$MARKER" ]; then
  echo "[entrypoint] Installing community node from $CUSTOM_PKG into $NODES_DIR (one-time)..."
  mkdir -p "$NODES_DIR"
  cd "$NODES_DIR"
  if [ ! -f package.json ]; then
    npm init -y
  fi
  # Local folder install so it shows as "Installed (Via npm)" in n8n.
  npm install "$CUSTOM_PKG" --omit=dev --no-package-lock \
    && echo "[entrypoint] Install OK." \
    || echo "[entrypoint] Install failed (continuing anyway)."
  cd - >/dev/null
else
